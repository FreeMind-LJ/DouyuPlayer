   ...
   public class ClientProxy extends Proxy implements IProxy
   {
      ...
      private function addJsCallback() : void
      {
         if(ExternalInterface.available)
         {
            ...
            ExternalInterface.addCallback("js_shieGiftEffect",this.hideGiftEffect);
            ...
            ExternalInterface.addCallback("js_page_fullscreen_state",this.js_page_fullscreen_state);
         ...
         EventCenter.addEventListener("WorkResponseData",this.__handlerWorkResponseData);
      }
      ...
      public function MyNewUser() : void
      {
         ...
         UserBehaviorLog.getInstance().addRealLog(UserBehaviorLog.POINT_ID_GATESERVER_CONNECT,{
            "ip":Param.ServerIp,
            "port":Param.ServerPort,
            "count":Param.Serversinfo.length,
            "first":(!!this.fristConnect?1:0)
         });
      ...
      private function hideGiftEffect(param1:int) : void
      {
         if(GlobalData.GIFT_EFFECT_LAYER)
         {
            GlobalData.GIFT_EFFECT_LAYER.shieldGift(param1);
         }
      }
      ...
      private function __handlerWorkResponseData(param1:ObjectEvent) : void
      {
         if(this._client != null)
         {
            this._client.guBak(param1.data);
         }
         else
         {
            $.jscall("console.log","Error! [%s]","workdata failed!!");
         }
      }
      ...
      private function js_turn_on_activity(param1:String, param2:Boolean) : void
      {
         ...
         if(param1 == "666")
         {
            if(Param.isYinghun || Param.isQQApp || Param.IsIndex || GlobalData.OldModel)
            {
               return;
            }
            if(param2)
      ...
      private function js_page_fullscreen_state(param1:int) : void
      {
         $.jscall("console.log","js_page_fullscreen_state",param1);
         EventCenter.dispatch("PageFullScreenState",{"state":param1});
      }
   }
}
