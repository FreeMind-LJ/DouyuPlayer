package com.greensock.loading
{
   ...
   public class LoaderMax extends LoaderCore
   {
      ...
      public static function parse(param1:*, param2:Object = null) : LoaderCore
      {
         var _loc3_:LoaderMax = null;
         var _loc4_:int = 0;
         var _loc5_:int = 0;
         var _loc6_:String = null;
         if(param1 is Array)
         {
            _loc3_ = new LoaderMax(param2);
            _loc4_ = param1.length;
            _loc5_ = 0;
            while(_loc5_ < _loc4_)
            {
               _loc3_.append(LoaderMax.parse(param1[_loc5_]));
               _loc5_++;
            }
            return _loc3_;
         }
         if(param1 is String)
         {
            _loc6_ = param1.toLowerCase().split("?")[0];
            _loc6_ = _loc6_.substr(_loc6_.lastIndexOf(".") + 1);
            if(_loc6_ in _extensions)
            {
               return new _extensions[_loc6_](param1,param2);
            }
         }
      ...
      public static function getLoader(param1:String) : LoaderCore
      {
      ...
      public function prependURLs(param1:String, param2:Boolean = false) : void
      {
         ...
         while(--_loc4_ > -1)
         {
            LoaderItem(this._loaders[_loc4_]).url = param1 + LoaderItem(this._loaders[_loc4_]).url;
         }
      }
      ...
      public function getLoader(param1:String) : LoaderCore
      {
      ...
      protected function _loadNext(param1:Event = null) : void
      {
         ...
         if(_status == LoaderStatus.LOADING)
         {
            ...
            while(true)
            {
               if(_loc5_ < _loc3_)
               {
                  _loc2_ = this._loaders[_loc5_];
                  if(!this.skipPaused && _loc2_.status == LoaderStatus.PAUSED)
                  {
                     break;
                  }
                  if(!this.skipFailed && _loc2_.status == LoaderStatus.FAILED)
                  {
                     super._failHandler(new LoaderEvent(LoaderEvent.FAIL,this,"Did not complete LoaderMax because skipFailed was false and " + _loc2_.toString() + " failed."));
                     return;
                  }
                  if(_loc2_.status <= LoaderStatus.LOADING)
                  {
                     _loc4_++;
                     if(!(_loc2_ in this._activeLoaders))
                     {
                        this._activeLoaders[_loc2_] = true;
                        _loc2_.addEventListener(LoaderEvent.COMPLETE,this._loadNext);
                        _loc2_.addEventListener(LoaderEvent.CANCEL,this._loadNext);
                        _loc2_.load(false);
                     }
                     if(_loc4_ == this.maxConnections)
                     {
                     }
                  }
                  _loc5_++;
               ...
               if(_loc4_ == 0 && _cachedBytesLoaded == _cachedBytesTotal)
               {
                  _completeHandler(null);
               }
            }
            super._failHandler(new LoaderEvent(LoaderEvent.FAIL,this,"Did not complete LoaderMax because skipPaused was false and " + _loc2_.toString() + " was paused."));
            return;
         }
      }
