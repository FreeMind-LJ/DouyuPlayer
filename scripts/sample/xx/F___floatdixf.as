package sample.xx
{
   import avm2.intrinsics.memory.li32;
   import avm2.intrinsics.memory.si32;
   import avm2.intrinsics.memory.lf64;
   
   public function F___floatdixf() : void
   {
      var _loc3_:* = 0;
      var _loc10_:* = 0;
      var _loc8_:* = 0;
      var _loc9_:* = 0;
      var _loc7_:int = 0;
      var _loc1_:* = 0;
      var _loc6_:* = NaN;
      var _loc2_:* = int(ESP);
      _loc3_ = _loc2_;
      _loc2_ = int(_loc2_ - 16);
      _loc1_ = li32(_loc3_ + 4);
      _loc10_ = li32(_loc3_);
      _loc9_ = _loc10_ | _loc1_;
      _loc6_ = 0;
      if(_loc9_ != 0)
      {
         _loc8_ = _loc1_ >> 31;
         _loc10_ = _loc8_ ^ _loc10_;
         _loc7_ = 1;
         if(uint(_loc10_) >= uint(_loc8_))
         {
            _loc7_ = 0;
         }
         _loc9_ = _loc8_ ^ _loc1_;
         _loc9_ = int(_loc9_ - _loc8_);
         _loc9_ = int(_loc9_ - _loc7_);
         _loc2_ = int(_loc2_ - 16);
         si32(_loc9_,_loc2_ + 4);
         var _loc5_:* = int(_loc9_ >>> 1);
         _loc5_ = _loc9_ | _loc5_;
         var _loc4_:* = int(_loc5_ >>> 2);
         _loc5_ = _loc5_ | _loc4_;
         _loc4_ = int(_loc5_ >>> 4);
         _loc5_ = _loc5_ | _loc4_;
         _loc4_ = int(_loc5_ >>> 8);
         _loc5_ = _loc5_ | _loc4_;
         _loc4_ = int(_loc5_ >>> 16);
         _loc5_ = _loc5_ | _loc4_;
         _loc5_ = _loc5_ ^ -1;
         _loc4_ = int(_loc5_ >>> 1);
         _loc4_ = _loc4_ & 1431655765;
         _loc4_ = int(_loc5_ - _loc4_);
         _loc5_ = _loc4_ & 858993459;
         _loc4_ = int(_loc4_ >>> 2);
         _loc4_ = _loc4_ & 858993459;
         _loc5_ = int(_loc5_ + _loc4_);
         _loc4_ = int(_loc5_ >>> 4);
         _loc5_ = int(_loc5_ + _loc4_);
         _loc5_ = _loc5_ & 252645135;
         _loc5_ = int(_loc5_ * 16843009);
         _loc1_ = int(_loc5_ >>> 24);
         _loc8_ = int(_loc10_ - _loc8_);
         si32(_loc8_,_loc2_);
         if(_loc9_ == 0)
         {
            _loc9_ = _loc8_ | int(_loc8_ >>> 1);
            _loc9_ = _loc9_ | int(_loc9_ >>> 2);
            _loc9_ = _loc9_ | int(_loc9_ >>> 4);
            _loc9_ = _loc9_ | int(_loc9_ >>> 8);
            _loc9_ = _loc9_ | int(_loc9_ >>> 16);
            _loc9_ = _loc9_ ^ -1;
            _loc5_ = int(_loc9_ >>> 1);
            _loc5_ = int(_loc9_ - (_loc5_ & 1431655765));
            _loc5_ = int(_loc5_ >>> 2);
            _loc9_ = int((_loc5_ & 858993459) + (_loc5_ & 858993459));
            _loc9_ = int(_loc9_ + (int(_loc9_ >>> 4)));
            _loc9_ = _loc9_ & 252645135;
            _loc9_ = int(_loc9_ * 16843009);
            _loc9_ = int(_loc9_ >>> 24);
            _loc1_ = int(_loc9_ + 32);
         }
         si32(_loc1_,_loc2_ + 8);
         ESP = _loc2_;
         F___ashldi3();
         _loc9_ = int(_loc3_ - 8);
         _loc9_ = _loc9_ | 4;
         _loc2_ = int(_loc2_ + 16);
         _loc5_ = int(eax);
         _loc4_ = int(edx);
         si32(_loc4_,_loc9_);
         si32(_loc5_,_loc3_ - 8);
         _loc6_ = lf64(_loc3_ - 8);
      }
      st0 = _loc6_;
      _loc2_ = _loc3_;
      ESP = _loc2_;
   }
}
