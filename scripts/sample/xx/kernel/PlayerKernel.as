package sample.xx.kernel
{
   import sample.xx.vfs.FileHandle;
   import sample.xx.CModule;
   import flash.utils.ByteArray;
   import sample.xx.vfs.PathUtils;
   import sample.xx.ram;
   import sample.xx.workerClass;
   import sample.xx.yield;
   ...
   public class PlayerKernel implements IKernel
   {
      ...
      public function open(param1:int, param2:int, param3:int, param4:int) : int
      {
         ...
         var _loc9_:Boolean = param2 & 512;
         var _loc8_:Boolean = _loc9_ && param2 & 2048;
         var _loc5_:int = -1;
         if(!_loc7_.length)
         {
         ...
         if(_loc12_)
         {
            if(param2 & (1 | 2 | 512 | 1024) && !_loc12_.writeable)
            {
               CModule.write32(param4,30);
               return -1;
            }
            ...
            _loc12_.writeable = param2 & 1 || param2 & 2;
      ...
      public function getdirentries(param1:int, param2:int, param3:int, param4:int, param5:int) : int
      {
         ...
         while(_loc9_ < _loc12_.length)
         {
            ...
            if(_loc6_.length + _loc11_ <= param3)
            {
               ...
               _loc13_.position = Number(_loc13_.position) + 1;
               _loc9_++;
               continue;
            }
            break;
      ...
      public function nanosleep(param1:int, param2:int, param3:int) : int
      {
         if(workerClass)
         {
            var _loc4_:uint = CModule.read32(param1 + 4);
